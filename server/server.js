...
app.use(
  session({
    secret:
      "qdwefwejfnHGCCfschGSVvTDSXSCStcvtctbctsctctthfTAgf7hthfg7FSTf6tF^SRHfh6f^SF6rf6CNSrc6rsbcb6rSc",
    resave: false,
...
async function run() {
  try {
      ...
      clientHelpers.cleanCardDatas(req.body).then((data) => {
        clientHelpers
          ...
          .then((response, card_data) => {
            res
              .status(200)
              .send({ redirect_url: `/create/preview/${data.clean_name}` });
            res.end();
          })
          .catch((err) => {
            console.log(err);
    ...
    app.post("/update/feedback/:name", (req, res, next) => {
      ...
      clientHelpers
        .updateFeedback(obj, client_db, req.params.name)
        .then((response) => {
          res.end();
        })
        .catch((err) => {
    ...
    app.post("/submit/customer-details/:name", (req, res, next) => {
      ...
      clientHelpers
        .updateCustomerDetails(obj, client_db, req.params.name)
        .then((response) => {
          res.end();
        })
        .catch((err) => {
    ...
    app.post("/update/view/:comp_name", (req, res, next) => {
      clientHelpers
        .updateViews(req.params.comp_name, client_db)
        .then((response) => {
          res.end();
        })
        .catch((err) => {
    ...
    app.post("/payment-successfull/:comp_name", (req, res, next) => {
      clientHelpers
        .afterPaymentCompleteProcessess(req.body, client_db)
        .then(() => {
          adminHelpers
            .updateCreatedCard(
              admin_db,
    ...
    app.post("/create/cancel-purchase/:comp_name", (req, res, next) => {
      clientHelpers
        .cancelPurchase(client_db, req.params.comp_name)
        .then(() => {
          res.json({ status: true });
        })
        .catch((err) => {
    ...
    app.post("/updatecard/:comp_name", (req, res, next) => {
      clientHelpers.updateCleanCardDatas(req.body).then((response) => {
        clientHelpers
          .updateCard(response, client_db, req.params.comp_name)
          .then(() => {
            res
              .status(200)
              .send({
    ...
    app.post("/manage/card/close-card", (req, res, next) => {
      paymentHelpers
        .cancelSubscription(req.body)
        .then(() => {
          clientHelpers
            .deleteCard(req.body, client_db)
            .then(() => {
              res.json({ status: true });
            })
            .catch((err) => {
    ...
    app.post("/franchisee/register", (req, res, next) => {
      createFranchisee(franchisee_db, req.body)
        .then(() => {
    ...
    app.post("/franchisee/login", (req, res, next) => {
      FranchiseeHelpers.loginFranchisee(req.body, franchisee_db)
        .then(() => {
          res.cookie("franchiseeEmail", req.body.email);
          res.cookie("isFranchiseeLogined", true);
    ...
    app.get("/get-franchisee-datas", (req, res, next) => {
      FranchiseeHelpers.getFranchisee(req.body, franchisee_db)
        .then((response) => {
    ...
    app.get("/get-franchisee-datas/:franchisee_email", (req, res, next) => {
      FranchiseeHelpers.getFranchisee(
        req.params.franchisee_email,
        franchisee_db
      )
        .then((response) => {
          res.json({ franchisee_data: response });
    ...
    app.post("/salary-payed/:franchisee_email", (req, res, next) => {
      adminHelpers
        .salaryPayed(franchisee_db, req.params.franchisee_email)
        .then(() => {
          res.json({ status: true });
        })
        .catch((err) => {
    ...
    app.post("/generate-completion", (req, res, next) => {
      aiHelpers
        .generateCompletion(req.body.prompt, req.body.temperature)
        .then((response) => {
