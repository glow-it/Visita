...
function CreateHeader({processIndex,loading,hideIndicators}) {

    let navigate = useNavigate()

  return (
    <header id='create-header' className={` w-full lg:h-16  ${hideIndicators == true ? 'h-16 -ml-6' : 'h-32'} lg:flex-row flex-col flex py-8 items-center bg-white justify-center fixed z-50 transition-shadow border-b shadow-md shadow-black/5`}>

       <div className={`absolute lg:left-8 flex  items-center justify-center lg:visible ${hideIndicators == true ? 'visible' : 'invisible'}`}>
       <img
       ...
       <div className={`create-process-indicator-wrapper flex ${hideIndicators == true ? 'invisible' : 'visible'}`}>
       ...
       <div className={`visita-text-animation ${hideIndicators == true ? 'hidden' : 'block'} `}>
