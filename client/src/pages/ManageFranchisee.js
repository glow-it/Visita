...
function ManageFranchisee() {
  ...
  let earnings_this_month =
    ...
    (createdCardsThisMonth.length * franchisee_profit)
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");

  let earnings_this_today =
    createdCardsToday &&
    (createdCardsToday.length * franchisee_profit)
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");

  let total_earnings =
    createdCards &&
    (createdCards.length * franchisee_profit)
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  ...
  useEffect(() => {
    ...
    axios
      .get(`${apiKeys.server_url}/get-franchisee-datas/${franchisee_email}`)
      .then((res) => {
        if (res.status) {
          if (franchisee_email) {
            ...
            document.title =
              capitalize(res.data.franchisee_data.franchisee_name) +
              " - Franchisee";
      ...
      <div className="flex flex-col w-full h-screen">
        <div className="w-full lg:min-h-[100px] min-h-[130px]  flex lg:flex-row flex-col items-center justify-center ">
          ...
          <h1
            ...
            className="font-bold lg:absolute lg:mt-0 mt-4 flex right-28 text-white bg-indigo-600 px-6 py-2 rounded-full hover:bg-indigo-700 cursor-pointer"
          >
              ...
              <TabPanel>
                <div className="w-full flex flex-col">
                  ...
                  <div className="w-full h-[330px] overflow-scroll flex flex-col lg:items-center mt-6 border-indigo-600">
                    {createdCardsThisMonth &&
                    createdCardsThisMonth.length == 0 ? (
                      <h1 className="text-2xl mt-16 text-indigo-400 font-medium">
                        No websites created this month
                      </h1>
                    ) : (
                      ""
                    )}
                    ...
                    {createdCardsThisMonth &&
                      createdCardsThisMonth.map((data, index) => {
                        return (
                          <div className="lg:min-w-[100%] min-w-[200%] pl-12 min-h-[64px] border border-indigo-100 rounded-xl shadow-sm shadow-black/5 flex items-center mt-4">
                            <h1 className="text-lg">
                              {index + 1}.{" "}
                              <span className="ml-4 text-indigo-600 capitalize">
                                {data.comp_name}
                              </span>
                            </h1>
                            <div className="flex-1  h-full  flex justify-end items-center pr-12">
                              <h1
                                onClick={() => navigate("/" + data.comp_name)}
                                className=" flex items-center justify-center text-2xl mr-4 cursor-pointer -rotate-45 text-indigo-600"
                              >
                              ...
                              <h1 className="text-slate-300 ml-6">
                                {data.created_date}
                              </h1>
                            </div>
                          </div>
              ...
              <TabPanel>
                <div className="w-full flex flex-col">
                  ...
                  <div className="w-full h-[330px] overflow-scroll flex flex-col lg:items-center mt-6 border-indigo-600">
                    ...
                    {createdCardsToday &&
                      createdCardsToday.map((data, index) => {
                        return (
                          <div className="lg:min-w-[100%] min-w-[200%] pl-12 min-h-[64px] border border-indigo-100 rounded-xl shadow-sm shadow-black/5 flex items-center mt-4">
                            <h1 className="text-lg">
                              {index + 1}.{" "}
